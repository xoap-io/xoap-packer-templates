...
variable "cd_files" {
  description = <<DOC
    ...
    This can include either files or directories; any directories will be copied onto the CD recursively, preserving directory structure hierarchy.
    Symlinks will have the link's target copied into the directory tree on the CD where the symlink was. File globbing is allowed.
  DOC
  sensitive   = false
  type        = list(string)
  default     = []
...
variable "cd_label" {
  ...
  sensitive   = false
  type        = string
  default     = "packer-cdrom"
}

variable "cdrom_adapter_type" {
  description = <<DOC
    The adapter type (or bus) that will be used by the cdrom device. This is chosen by default based on the disk adapter type.
    VMware tends to lean towards ide for the cdrom device unless sata is chosen for the disk adapter and so Packer attempts to mirror this logic.
    This field can be specified as either ide, sata, or scsi.
  DOC
  sensitive   = false
  type        = string
  default     = "sata"
...
variable "vmx_data" {
  description = "Arbitrary key/values to enter into the virtual machine VMX file. This is for advanced users who want to set properties that aren't yet supported by the builder."
  sensitive   = false
  type        = map(string)
  default     = {}
}
